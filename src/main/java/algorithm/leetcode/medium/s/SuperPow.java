...
public class SuperPow {
    ...
    public int superPow(int a, int[] b) {
        ...
        for (int i = 0; i < length; i++) {
            finalB += this.getPow(10,length-1-i) * b[i];
        }
        if (a == 1) {
            return 1;
        }
        ...
        // 到这里 a > 1 且 a != 1337  且 finalB > 0
        if (a > 1337) {
            return superPow(a%1337,b);
        } else {
            if (finalB % 2 == 0) {
                return superPow(a*a,this.getArray(finalB/2));
            } else {
                return superPow(a*a,this.getArray(finalB/2)) * a % 1337;
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int i = superPow.superPow(2, new int[]{3});
        System.out.println(i);

        int i1 = superPow.superPow(2, new int[]{1,0});
        System.out.println(i1);

        int i2 = superPow.superPow(1, new int[]{4,3,3,8,5,2});
        System.out.println(i2);

        int i3 = superPow.superPow(2147483647, new int[]{2,0,0});
